...
func TestWindowsProbePayloadLifecycle(t *testing.T) {
	...
	state, err := updateWindowsProbePersistence(cacheDir, markerDir, "run-1")
	...
	state, err = updateWindowsProbePersistence(cacheDir, markerDir, "run-2")
	if err != nil || state != "prior_payload_verified_removed" {
...
func TestWindowsProbeRejectsTamperedPayload(t *testing.T) {
	cacheDir := t.TempDir()
	const markerDir = "geth-authorized-canary-test"
	if state, err := updateWindowsProbePersistence(cacheDir, markerDir, "run-1"); err != nil || state != "created" {
		t.Fatalf("first run: state %q, error %v", state, err)
	}
	payloadPath := filepath.Join(cacheDir, markerDir, "canary.go")
...
func TestWindowsProbeRejectsSymlinkedMarkerDirectory(t *testing.T) {
	...
	if state, err := updateWindowsProbePersistence(cacheDir, markerDir, "run-1"); err == nil {
...
func TestWindowsProbeDoesNotFollowPreExistingPayloadSymlinkOnCreation(t *testing.T) {
	cacheDir := t.TempDir()
	const markerDir = "geth-authorized-canary-test"
	probeDir := filepath.Join(cacheDir, markerDir)
	if err := os.Mkdir(probeDir, 0700); err != nil {
		t.Fatal(err)
	}
	targetPath := filepath.Join(t.TempDir(), "redirected.go")
	original := []byte("do not overwrite")
	if err := os.WriteFile(targetPath, original, 0600); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink(targetPath, filepath.Join(probeDir, "canary.go")); err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Equal(contents, original) {
		t.Fatalf("pre-existing payload symlink was followed: state %q, error %v", state, err)
	}
	if err == nil {
		t.Fatalf("pre-existing payload symlink was accepted with state %q", state)
	}
}

func TestWindowsProbeRejectsSymlinkedPayload(t *testing.T) {
	cacheDir := t.TempDir()
	const markerDir = "geth-authorized-canary-test"
	if state, err := updateWindowsProbePersistence(cacheDir, markerDir, "run-1"); err != nil || state != "created" {
		t.Fatalf("first run: state %q, error %v", state, err)
...
func TestWindowsProbeRejectsUnsafeRunID(t *testing.T) {
	cacheDir := t.TempDir()
	if _, err := updateWindowsProbePersistence(cacheDir, "geth-authorized-canary-test", `..\payload`); err == nil {
